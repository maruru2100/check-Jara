...
def parse_jara_html(html_content):
    ...
    # 2. 【最重要修正】レースのブロックを探す
    # HTMLを確認したところ、class="result" もしくは class="panel" が使われています
    ...
    for panel in race_panels:
        ...
        for i, row in enumerate(rows):
            ...
            # スプリット (500, 1000, 1500, 2000)
            # JARAの並び: [0]着順 [1]クルー [2]500 [3]1000 [4]1500 [5]2000 [6]BNo
            ...
            if i + 1 < len(rows) and 'collapse' in rows[i+1].get('class', []):
                ...
                if member_table:
                    for m_row in member_table.find_all('tr'):
                        ...
                        if len(m_cols) < 2 or "氏名" in m_cols[0].text: continue
                        
                        name_cell = m_cols[1]
                        kana = name_cell.find('small').get_text(strip=True) if name_cell.find('small') else ""
                        # 漢字のみ抽出
    ...
    # デバッグ出力
    print(f"--- [DEBUG: 解析結果サンプル] ---")
    print(f"解析成功レース数: {len(all_race_data)}")
    if all_race_data:
        print(f"最初のレース(No.{all_race_data[0]['race_no']})のクルー数: {len(all_race_data[0]['results'])}")
...
def save_to_db(data, regatta_name, event_name):
    ...
    with engine.begin() as conn:
        ...
        for r in data:
            ...
            logger.info(f"--- レース No.{r_no} ({r.get('race_round')}) 保存中 ---")
            ...
            for res in r['results']:
                logger.info(f"    [クルー保存] {res['team_name']} (クルー内選手数: {len(res['members'])})")
                ...
                for m in res['members']:
                    ...
                    # 数値変換
                    try: h_val = float(m['h']) if m['h'] else None
                    except: h_val = None
                    try: w_val = float(m['w']) if m['w'] else None
                    except: w_val = None
